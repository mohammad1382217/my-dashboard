...
const triggerClass =
  'inline-flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-slate-700 outline-none transition-[color,background-color,border-color,box-shadow,transform] hover:bg-slate-100 focus-visible:bg-slate-100 focus-visible:ring-2 focus-visible:ring-primary-500/40 active:scale-[0.98] data-[open=true]:bg-slate-100 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:focus-visible:bg-zinc-800 dark:data-[open=true]:bg-zinc-800'
...
export const NavigationMenu = forwardRef<HTMLElement, NavigationMenuProps>(function NavigationMenu(
  ...
  function positionFor(index: number): Position | null {
    ...
    // RTL: anchor the panel to the trigger's right edge so it opens toward the left.
      ...
      {openIndex >= 0 && position && openContent
        ? createPortal(
            <div
              ...
              className="fixed z-50 w-72 rounded-xl border border-slate-200 bg-white p-2 shadow-lg ring-1 ring-black/5 dark:border-zinc-800 dark:bg-zinc-900 dark:ring-white/10"
            >
              {isLinkList(openContent) ? (
                <ul className="flex flex-col">
                  {openContent.map((link, i) => (
                    <li key={i}>
                      <a href={link.href} className="block rounded-lg p-3 outline-none transition-[color,background-color,border-color,box-shadow,transform] hover:bg-slate-100 focus-visible:bg-slate-100 focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:hover:bg-zinc-800 dark:focus-visible:bg-zinc-800">
