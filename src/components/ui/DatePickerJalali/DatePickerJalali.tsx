...
const triggerBase =
  'flex h-10 w-full items-center justify-between gap-2 rounded-md border bg-white px-3 text-sm outline-none transition-[color,background-color,border-color,box-shadow,transform] disabled:cursor-not-allowed disabled:opacity-60 dark:bg-zinc-900'
const triggerNormal =
  'border-slate-300 text-slate-900 focus-visible:border-primary-500 focus-visible:ring-2 focus-visible:ring-primary-500/30 dark:border-zinc-700 dark:text-zinc-100'
const triggerError =
...
const navButton =
  'grid size-7 place-items-center rounded-md text-slate-500 outline-none transition-[color,background-color,border-color,box-shadow,transform] hover:bg-slate-100 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-primary-500/40 active:scale-[0.98] dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100'
const headerLabel =
  'rounded-md px-2 py-1 text-sm font-medium text-slate-900 outline-none transition-[color,background-color,border-color,box-shadow,transform] hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-primary-500/40 active:scale-[0.98] dark:text-white dark:hover:bg-zinc-800'
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div ref={wrapperRef} className="relative">
        ...
        >
          <span className={formatted ? '' : 'text-faint'}>{formatted ?? placeholder}</span>
                ...
                <>
                  <div className="mb-1 grid grid-cols-7">
                    {WEEKDAYS.map((weekday) => (
                      <span key={weekday} className="grid h-7 place-items-center text-xs font-medium text-faint">
                        {weekday}
                      </span>
                    ))}
                  </div>
                    ...
                    {cells.map((day, index) =>
                      day === null ? (
                        ...
                        <button
                          ...
                          className={twMerge(
                            'grid size-8 place-items-center rounded-md text-sm outline-none transition-[color,background-color,border-color,box-shadow,transform] focus-visible:ring-2 focus-visible:ring-primary-500/40 active:scale-[0.98]',
                            selectedJ && isSameDay(selectedJ, view.jy, view.jm, day)
                              ? 'bg-primary-600 text-white'
                              : isSameDay(todayJ, view.jy, view.jm, day)
                                ? 'text-primary-600 ring-1 ring-inset ring-primary-300 hover:bg-primary-50 dark:text-primary-400 dark:ring-primary-500/50 dark:hover:bg-primary-500/10'
                ...
                <div className="grid grid-cols-3 gap-1">
                  {MONTHS.map((monthName, index) => (
                    <button
                      ...
                      className={twMerge(
                        'rounded-md px-2 py-2 text-sm outline-none transition-[color,background-color,border-color,box-shadow,transform] focus-visible:ring-2 focus-visible:ring-primary-500/40 active:scale-[0.98]',
                        view.jm === index + 1
                          ? 'bg-primary-600 text-white'
                          : 'text-slate-700 hover:bg-slate-100 dark:text-zinc-300 dark:hover:bg-zinc-800',
                      )}
