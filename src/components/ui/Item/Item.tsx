...
export const Item = forwardRef<HTMLDivElement, ItemProps>(function Item(
  ...
  return (
    <div
      ref={ref}
      className={twMerge(
        'flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 text-sm dark:border-zinc-800 dark:bg-zinc-900',
        interactive ? 'cursor-pointer transition-colors hover:bg-slate-50 dark:hover:bg-zinc-800' : null,
        className,
      )}
      {...props}
    >
      {leading ? <span className="shrink-0 text-slate-500 dark:text-zinc-400">{leading}</span> : null}
      <div className="flex min-w-0 flex-1 flex-col">
        {children ?? (
          <>
            {title ? <span className="truncate font-medium text-slate-900 dark:text-zinc-100">{title}</span> : null}
            {description ? <span className="truncate text-slate-500 dark:text-zinc-400">{description}</span> : null}
          </>
        )}
      </div>
      {trailing ? <span className="shrink-0 text-slate-500 dark:text-zinc-400">{trailing}</span> : null}
    </div>
  )
})
